...
public class ABB<K, V> implements IMapeamento<K, V> {
    ...
    public Lista<V> recortar(K chaveDeOnde, K chaveAteOnde) {
        ...
        while (atual != null || !pilha.isEmpty()) {
            ...
            atual = pilha.pop();
            if (comparador.compare(chaveDeOnde, atual.getChave()) <= 0 &&
                comparador.compare(chaveAteOnde, atual.getChave()) >= 0) {
                resultado.inserir(atual.getItem());
            }
    
            if (comparador.compare(chaveAteOnde, atual.getChave()) < 0) {
                break;
            }
